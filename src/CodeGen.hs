...
instance Pretty Exp where
  ...
  ppe (Let binds e) = parens $ text "let" <+> parens (hsep $ map ppe binds) <+> ppe e
  ppe (Letrec binds e) = parens $ text "letrec" <+> parens (hsep $ map ppe binds) <+> ppe e
  ppe (As e t) = parens $ ppe e <+> char ':' <+> ppe t
  ppe (Begin es e) = parens $ text "begin" <+> hsep (map ppe es) <+> ppe e
  ppe (Repeat x e1 e2 e) = parens $ text "repeat" <+> parens (text x <+> ppe e1 <+> ppe e2) <+> ppe e
