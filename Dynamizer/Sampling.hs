...
sampleOne (p, ts) (inf &&& sup -> (mn, mx)) g = fst <$> foldM f ([], (p, 0, g)) ts
  where
    ...
    f x@(l, (potential, used, gen)) ct@(Ann (c, a) _) =
...
sampleLessPreciseType :: forall a. Ann (Int, a) Type
                      -> Int
                      -> RVar (Maybe (Ann a Type))
sampleLessPreciseType s = runMaybeT . f s
  where
    f :: Ann (Int, a) Type -> Int -> MaybeT RVar (Ann a Type)
    f (Ann (_, a) Dyn)          n | n == 0    = return $ Ann a Dyn
                                  | otherwise = mzero
    f (Ann (_, a) BlankTy)      n | n == 0    = return $ Ann a BlankTy
                                  | otherwise = mzero
    f (Ann _ (ArrTy _ _))       0 = mzero
    f t@(Ann (n', a) _)         n | n == n' = return $ stripCount t
                                  | n == 0  = return $ Ann a Dyn
    f (Ann (_, a) CharTy)       n | n == 1    = return $ Ann a CharTy
                                  | otherwise = mzero
    f (Ann (_, a) IntTy)        n | n == 1    = return $ Ann a IntTy
                                  | otherwise = mzero
    f (Ann (_, a) FloatTy)      n | n == 1    = return $ Ann a FloatTy
                                  | otherwise = mzero
    f (Ann (_, a) BoolTy)       n | n == 1    = return $ Ann a BoolTy
                                  | otherwise = mzero
    f (Ann (_, a) UnitTy)       n | n == 1    = return $ Ann a UnitTy
                                  | otherwise = mzero
    ...
    f (Ann (c, a) (FunTy ts t)) n = do
      (rt':ts') <- sampleTypeList (n-1) (c-1) (t:ts)
      return $ Ann a $ FunTy ts' rt'
    f (Ann (c, a) (ArrTy ts t)) n = do
      (rt':ts') <- sampleTypeList n c (t:ts)
      return $ Ann a $ ArrTy ts' rt'
    f (Ann (c, a) (TupleTy ts)) n = Ann a . TupleTy <$> sampleTypeList (n-1) (c-1) ts

    sampleTypeList :: Int -> Int -> [Ann (Int, a) Type] -> MaybeT RVar [Ann a Type]
    sampleTypeList n p ts = do
      guard $ n <= p
      l <- lift $ weightedShuffle $ map (\y@(_, Ann (x, _) _) -> (x, y)) $ zip [0..] ts
      mapM snd $ sortBy (compare `on` fst) $ fst $ foldl' g ([], (p, 0, seedTFGen (0, 11, 22, 33))) l
        where
          g :: forall g. RandomGen g
            ...
            -> (Int, Ann (Int, a) Type)
            -> ([(Int, MaybeT RVar (Ann a Type))], (Int, Int, g))
          g (l', (potential, used, gen)) (i, t@(Ann (c, _) _)) =
            let potential' = potential-c
